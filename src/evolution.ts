...
export class EvolutionService {
  ...
  async decide(input: EvolutionInput): Promise<EvolutionResult> {
    ...
    if (decision === 'UPDATE_NEIGHBOR' || decision === 'STRENGTHEN_AND_UPDATE') {
      const updateResponse = await this.llm(
        UPDATE_NEIGHBORS_PROMPT
          ...
          .replace('{neighbor_count}', String(input.neighbors.length)),
        { temperature: 0.2 },
      );
      ...
      updatedNeighbors = input.neighbors.map((n, i) => {
        ...
        return {
          ...
          tags: (upd.tags && upd.tags.length > 0 ? upd.tags : n.tags) as string[],
        };
      });
    }
  ...
  private renderNeighbors(neighbors: MemoryNote[]): string {
...
function parseDecision(response: string): { decision: EvolutionDecision; reason: string } {
  try {
    const cleaned = (response.trim().match(/```(?:json)?\s*\n?([\s\S]*?)\n?\s*```/) ?? [null, response.trim()])[1];
    const json = JSON.parse(cleaned);
    if (json && typeof json === 'object') {
      if (json.decision) return { decision: normalizeDecision(json.decision), reason: json.reason ?? '' };
      const should = json.should_evolve;
      const actions = json.actions ?? [];
      if (!should) return { decision: 'NO_EVOLUTION', reason: '' };
      const hasStrengthen = actions.includes('strengthen');
      const hasUpdate = actions.includes('update_neighbor');
      if (hasStrengthen && hasUpdate) return { decision: 'STRENGTHEN_AND_UPDATE', reason: '' };
      if (hasStrengthen) return { decision: 'STRENGTHEN', reason: '' };
      if (hasUpdate) return { decision: 'UPDATE_NEIGHBOR', reason: '' };
    }
  } catch {
    // fall through
  }
...
function normalizeDecision(text: string): EvolutionDecision {
...
function parseStrengthen(response: string): { connections: number[]; tags: string[] } {
  try {
    const cleaned = (response.trim().match(/```(?:json)?\s*\n?([\s\S]*?)\n?\s*```/) ?? [null, response.trim()])[1];
    const json = JSON.parse(cleaned);
    if (json && typeof json === 'object') {
...
function parseUpdateNeighbors(response: string, n: number): Array<{ context: string; tags: string[] }> {
  const out: Array<{ context: string; tags: string[] }> = [];
  for (let i = 0; i < n; i++) {
...
function _extractSection(text: string, marker: string, nextMarkers: string[]): string {
