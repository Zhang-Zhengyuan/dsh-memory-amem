...
function parseAnalysis(response: string, original: string): MemoryAnalysis {
  // Try JSON first
  try {
...
function parseList(input: unknown): string[] {
  if (Array.isArray(input)) return input.filter((x): x is string => typeof x === 'string').map((s) => s.trim()).filter(Boolean);
...
function heuristicContext(content: string): string {
