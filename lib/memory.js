...
export class AgenticMemoryEngine {
    ...
    init() {
        if (this.disposed)
            return Promise.reject(new Error('Memory engine is disposed'));
    ...
    async add(content, opts = {}) {
        ...
        const evolution = this.config.enableEvolution && this.llm.available !== false && neighbors.length > 0
            ...
            : { decision: 'NO_EVOLUTION', reason: 'Evolution disabled or no relevant neighbors' };
        const now = Date.now();
        const note = {
            id: uuid(),
    ...
    matchesScope(note, sessionId) {
        if (this.config.memoryScope === 'global')
            return true;
    ...
    retrieveNeighbors(analysis, k, sessionId) {
        ...
        return this.retriever.retrieveScored(query, this.notes.size)
            ...
            .filter((result) => result.note !== undefined && this.matchesScope(result.note, sessionId))
            .slice(0, k);
    }
    async applyEvolution(note, evolution, neighbors) {
        ...
        if (decision === 'STRENGTHEN' || decision === 'STRENGTHEN_AND_UPDATE') {
            ...
            if (evolution.updatedTags?.length)
        ...
        if (decision === 'UPDATE_NEIGHBOR' || decision === 'STRENGTHEN_AND_UPDATE') {
            for (const update of evolution.updatedNeighbors ?? []) {
                ...
                if (update.tags?.length)
                    target.tags = Array.from(new Set([...target.tags, ...update.tags]));
    ...
    async loadFromDisk() {
        ...
        try {
            ...
            if (errorCode(error) !== 'ENOENT')
                this.log.warn(`Failed to list notes: ${errorMessage(error)}`);
            return;
        }
    ...
    scheduleFlush() {
        this.flushTimer = setInterval(() => {
            if (this.persistedRevision < this.revision) {
                void this.flush().catch((error) => this.log.error(`flush failed: ${errorMessage(error)}`));
            }
    ...
    async writeSnapshot(targetRevision) {
        ...
        for (const note of notes)
            await atomicWrite(path.join(notesDir, `${note.id}.json`), JSON.stringify(note));
